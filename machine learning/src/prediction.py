...
def predict(model, device, test_data_loader, include_token_type_ids=False):
    ...
    recall = recall_score(y_true=labels, y_pred=predicted_class)
    ...
    return simple_accuracy, f1, recall, report, scores, pd.DataFrame({'predictions': predicted_class, 'labels': labels})
